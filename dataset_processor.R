...
#-------------------------
tmdb_raw <- read_lines("http://files.tmdb.org/p/exports/tv_series_ids_09_29_2022.json.gz")
tmdb <- tmdb_raw %>% 
...
imdb_e_trimmed <- imdb_e %>%
	...
	filter(mean(num_votes,na.rm = TRUE) >= 10) # Take only shows whose *rated* episodes have a mean >= 10
